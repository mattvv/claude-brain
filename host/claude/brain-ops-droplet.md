...
quote a headroom figure for them. As Claude headroom falls, route implementation
and bulk reading through `brain-*` consultants instead of doing it in-session; at the
reserve the guard hook blocks Anthropic-backed subagents so the session keeps enough quota
to answer. If the user asks to spend the reserve anyway, `brain usage override <minutes>`
lifts the block, and `brain config usage <advisory|block|off>` / `reserve <pct>` changes the
policy for good.
